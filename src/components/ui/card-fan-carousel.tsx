      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer w-64 h-80 absolute top-1/2 left-1/2 -mt-40 -ml-32">{image}</a>
            ) : (
              <div key={index} className="fan-card w-64 h-80 absolute top-1/2 left-1/2 -mt-40 -ml-32">{image}</div>
            );
          })}
        </div>
      </div>
